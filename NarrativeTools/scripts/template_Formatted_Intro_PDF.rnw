...
\includegraphics[width=0.50\textwidth]{Cryptobranchus_alleganiensis.jpg}
 \caption{Eastern hellbender is the state amphibian of Pennsylvania. Photographer: Pete Woods, PNHP}
...
\caption{Physiographic provinces and sections of \Sexpr{nameCounty} County.}
\label{fig:PhysProv}
\end{figure}
...
\caption{Watersheds of \Sexpr{nameCounty} County.}
...
\includegraphics[width=0.50\textwidth]{AMDdischarge.jpg}
 \caption{Abandoned mine drainage (AMD) is polluted water that leaches out of mines, often turning streams a characteristic orange color from iron oxides. Photographer: PNHP}
...
\caption{Forest patches of \Sexpr{nameCounty} County.}
\label{fig:Landcover}
\end{figure}
...
\caption{Spotted lanternfly, a rapidly spreading invasive insect in southwestern Pennsylvania. Photographer: MTSOfan, Creative Commons}
\label{fig:invasive}
% \end{figure}
\end{wrapfigure}
...
\includegraphics[width=0.50\textwidth]{NHA_Example}
 \caption{Definition of an Natural Heritage Area}
 \label{fig:NHAdef}
% \end{figure}
\end{wrapfigure}
...
\includegraphics[width=0.50\textwidth]{NHA_siteacct_Page_1.jpg}
 \caption{Example of an Natural Heritage Area site account.}
 \label{fig:NHAact}
% \end{figure}
\end{wrapfigure}
...
\begin{table}[h!]
  \begin{center}
    \caption{Pennsylvania Legal Status.}
    \label{tab:statusPA}
    \begin{tabular}{C{2cm}p{12cm}} 
      \hline
...
\caption{Natural Heritage Areas of \Sexpr{nameCounty} County.}
\label{fig:NHAmap}
\end{figure}

\noindent The total area of NHA overlapping \Sexpr{nameCounty} County is \Sexpr{nha_area} acres. \Sexpr{stringr::str_to_sentence(words(nrow(NHA_ProtectedLands_sum)))} NHAs have a portion of their area protected; totaling \Sexpr{format(sum(NHA_ProtectedLands_sum$ACRESprotected), big.mark=",")} acres. \Sexpr{stringr::str_to_sentence(words(nrow(NHA_ProtectedLands_sum[which(NHA_ProtectedLands_sum$Percent>=90),])))} NHAs have 90\% of their area under some form of protection. Please consult an individual NHA for more detailed information about protection status.


\newpage
\begin{longtable}{C{0.5in}p{2in}C{1in}p{3in}}
\caption{Natural Heritage Areas categorized by significance. The results of the Natural Heritage Inventory are summarized in order of their ecological significance based on the number and degree of rarity of species they support. Significance ranks are \textit{Global}, \textit{Regional}, \textit{State}, and \textit{Local}.}
\label{tab:countyNHA}\\
\hline
...
\includegraphics[width=0.50\textwidth]{Steve Grund, wildflower walk, Great Gorge Trail Ohiopyle.jpg}
 \caption{Conversation and community engagement are important conservation tools. Photographer: Christopher Tracey, PNHP}
 \label{fig:Biodversity}
% \end{figure}
\end{wrapfigure}
