...
\begin{wrapfigure}{R}{.5\textwidth}
 \fbox{\begin{minipage}{\dimexpr\linewidth-2\fboxrule-2\fboxsep}
    ...
    %\centering
    %\includegraphics[width=0.48\textwidth]{PACE}}
...
@
\noindent \Sexpr{NatHistOverview$NatHistOpeningPara} \\
...
Every stream, tributary, and river has an associated watershed, with small watersheds merging to become larger watersheds. Watersheds can be classified at large scales, like that of the Allegheny River, or smaller scales, like that of Buffalo Creek, but all land is part of a watershed. % Susquehanna Raystown Branch of the Juniata River
  ...
  for(i in 1:nrow(speciestable)){
    if(speciestable$ELEMENT_TYPE[i] == t){
      ...
      if(is.na(speciestable$SCOMNAME[i])){
        speciesstring <- paste("no common name (\\textit{", speciestable$SNAME[i], "}) ", sep="")
      } else if(speciestable$SCOMNAME[i]==speciestable$SNAME[i]){
        speciesstring <- paste0(speciestable$SCOMNAME[i])
      } 
      else {
        speciesstring <- paste(speciestable$SCOMNAME[i], " (\\textit{", speciestable$SNAME[i], "}) ", sep="")
...
\begin{wrapfigure}{r}{.5\textwidth}
 \fbox{\begin{minipage}{\dimexpr\linewidth-2\fboxrule-2\fboxsep}
  \begin{center}
   \includegraphics[width=0.5\textwidth]{waplogo}
  \end{center}
 %\begin{flushleft}
   \noindent The Pennsylvani awildlife Action Plan...\\
   
 \noindent Stuff stuff. \\
 
 \noindent Sign up for your free iMapInvasives user account today by going to https://wildlifeactionmap.pa.gov and clicking on the “Login” button. \\
  %\end{flushleft}
  \end{minipage}}
\end{wrapfigure}
