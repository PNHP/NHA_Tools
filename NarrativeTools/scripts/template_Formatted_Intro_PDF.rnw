...
\noindent We do some fieldwork.  Fieldwork begins with planning a survey. This survey can be based on previously known reports (museum records, various databases, etc).

\noindent Data is entered into a GIS-based database known as "Biotics".
...
All of the above species are mapped within an NHA, except for the \Sexpr{SUSN$SNAME}.
