...
\noindent Federally listed species are under the jurisdiction of the US Fish and Wildlife Service and have the status definition as described in Table~\ref{table:statusFed}. 
...
\noindent Each population of a species of concern is assigned a quality rank, based on the estimated probability that it will persist over time at the site. The most commonly assigned ranks are summarized in Table ~\ref{table:rankQuality}. 
