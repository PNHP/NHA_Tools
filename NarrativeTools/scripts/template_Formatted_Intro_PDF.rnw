...
for(t in taxa){
  if(t=="AM"){
    ...
    cat(paste0(' \\midrule \\multicolumn{4}{l}{','{\\textbf{Beetles }}',"\\includegraphics[width=0.15in]{",taxaicon[which(taxaicon$ELEMENT_TYPE==t),"icon"],"}}", '\\\\ \\midrule \n'))
  }
else if(t=="CGH"|t=="O"){
  ...
  for(i in 1:nrow(speciestable)){
    if(speciestable$ELEMENT_TYPE[i] == t){
      # check to see if the SNAME is the same asthe SCOMNAME and if so, please drop the SNAME
      if(speciestable$SCOMNAME[i]==speciestable$SNAME[i]){
