...
\begin{wrapfigure}[13]{R}{0.5\textwidth}
  \centering
  \includegraphics[0.5\textwidth]{nha_photos$P1F}
  \captionsetup{labelformat=empty, justification=centerlast}
  <<label=captionfunction, echo=FALSE, results='asis'>>=
  capfun <- as.character(paste(nha_photos$P1C, "\\textcolor{gray}{", nha_photos$P1N,"}", sep=" "))
  @
   ...
   \centering
    \begin{tabular}{p{3in}cccccc}
     ...
     for(i in 1:nrow(speciestable)){ 
       ...
       cat(paste0(speciesstring, " &", "\\includegraphics[width=0.15in]{",speciestable$Images[i],"}","&",speciestable$GRANK[i], "&", speciestable$SRANK[i],"&", paste0(speciestable$SPROT[i]," (",speciestable$PBSSTATUS[i],")"), "&", speciestable$LASTOBS[i],"&", speciestable$EORANK[i],"\\\\") )
     }
@       
...
if(is.na(nha_photos$P2F) & is.na(nha_photos$P3F)) {
  photos <- paste0("\\\\")
    ...
    photos <- paste0("\\begin{figure}[h!] \\centering \\includegraphics[width=\\linewidth]{\\Sexpr{p2_path}} \\captionsetup{labelformat=empty, justification=centerlast} \\caption{\\Sexpr{capfun2}} \\end{figure}")
  } else {
