...
This site has been rated as \Sexpr{nha_siterank} significant due to
...
@
\noindent
All species tracked by PNHP present at this NHA include:
\smallskip
\afterpage{\clearpage}
% Species Table
...
\textbf{Previous CNHI reference:} This site replaces the following previously published sites: \Sexpr{selected_nha$OLD_SITE_NAME} \\
\textbf{Associated NHAs:} This site is associated with the following other NHAs: \Sexpr{selected_nha$ASSOC_NHA}.  We recommend consulting the accounts for those sites for additional conservation information. \\ 
